...
with schemdraw.Drawing(show=False) as d:
    ...
    cat_2MASS  = flow.Data(h=1, w=2).at((2.5, -2)).label('2MASS')
    cat_PS1    = flow.Data(h=1, w=2).at((4.75, -3)).label('PS1')
    cat_AW     = flow.Data(h=1, w=2).at((7, -4)).label('AW')
    ...
    ir_opt     = flow.Box(h=1.0, w=2.5).at((0, -6.5)).label('COMPILE\nFEATURES')
    elm.Wire('-|', arrow='->').at(cat_2MASS.W).to(ir_opt.N)
    elm.Wire('-|', arrow='->').at(cat_PS1.W).to(ir_opt.N)
    elm.Wire('-|', arrow='->').at(cat_AW.W).to(ir_opt.N).label(r'$\mathtt{band\_num}$', loc='left')
    elm.Wire('-|', arrow='->').at(cat_radio.E).to(ir_opt.N).label(r'$\mathtt{radio\_detect}$', loc='top')
    elm.Wire('-|', arrow='->').at(cat_class.E).to(ir_opt.N).label(r'$\mathtt{class,Z}$', loc='top')
    elm.Wire('-', arrow='->').at(init.S).to(ir_opt.N)
    impute     = flow.Box(h=1.0, w=2.5).at((0, -8.5)).label('IMPUTE\nMAGNITUDES')
    elm.Wire('-', arrow='->').at(ir_opt.S).to(impute.N)
    ...
    correlat   = flow.Box(h=1.0, w=2.5).at((0, -16.5)).label('CORRELATION\nFACTOR')
    pps        = flow.Box(h=1.0, w=2.5).at((0, -18.5)).label('PREDICTIVE\nPOWER SCORE')
    elm.Wire('-', arrow='->').at(correlat.S).to(pps.N)
    feat_sel = elm.EncircleBox([correlat, pps], padx=.1, pady=-.1).linestyle('-').linewidth(2).label('FEATURE\nSELECTION', ofst=(2.8, -3.4), rotate=270)
    feat_sel.at((0, -18.0))
